...
//CUSTOM CHANGES
//removed value surroundig with '' if value contains %. This is for URI, tags and other yaml features, normally, yaml creates full class or object if encouters %TAG notation. Here it was creating a quated string 
 ...
 */
  ...
  private function _dumpNode($key, $value, $indent, $previous_key = -1, $first_key = 0, $source_array = null) {
   ...
   //original   strpos($value,"*") !== false || strpos($value,"#") !== false || strpos($value,"<") !== false || strpos($value,">") !== false || strpos ($value, '%') !== false || strpos ($value, '  ') !== false ||
            //removed % notation
 strpos($value,"*") !== false || strpos($value,"#") !== false || strpos($value,"<") !== false || strpos($value,">") !== false ||strpos ($value, '  ') !== false ||
    ...
    if (is_array ($source_array) && array_keys($source_array) === range(0, count($source_array) - 1)) {
      ...
      //does not make difference // if (strpos($key, ":") !== false || strpos($key, "#") !== false) { $key =  $key ; }
  ...
  private function _doLiteralBlock($value,$indent) {
    ...
    $exploded = explode("\n",$value);
    $newValue = '|';
    if (!empty($exploded[0]) && ($exploded[0] == "|" || $exploded[0] == "|-" || $exploded[0] == ">")) {
        $newValue = $exploded[0];
        unset($exploded[0]);
    }
    $indent += $this->_dumpIndent;
  ...
  private function _doFolding($value,$indent) {
    ...
    if ($this->_dumpWordWrap !== 0 && is_string ($value) && strlen($value) > $this->_dumpWordWrap) {
      ...
      if (is_numeric($value) && is_string($value)) {
          if($this->rawy){ } //my-addition to ge traw yaml
          else { $value = '"' . $value . '"'; }
      }
    }
...
// LOADING FUNCTIONS
  ...
  private function referenceContentsByAlias ($alias) {
    do {
      if (empty($this->SavedGroups[$alias])) { echo "Bad group name: $alias."; break; }
  ...
  private function addArray ($incoming_data, $incoming_indent) {
   ...
   // //print_r ($incoming_data);
    ...
    $value = !empty($incoming_data[$key]) ? $incoming_data[$key] : null;
